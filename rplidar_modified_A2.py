...
class RPLidar(object):
    ...
    def motor_setPWM(self, PWM):
        #the order of the 2 bytes has to be swapped. Eg: 660 = 0x0294 but the sending sequence has to be 0x94 0x02
        PWM_bytes = PWM.to_bytes(2,'big')
        PWM_payload_size = b'\x02'
        PWM_payload_data = PWM_bytes[1].to_bytes(1,'big') + PWM_bytes[0].to_bytes(1,'big')
        PWM_checksum = int.from_bytes(SYNC_BYTE,'big') ^ int.from_bytes(MOTOR_START_BYTE,'big') ^ int.from_bytes(PWM_payload_size,'big') ^ PWM_bytes[0] ^ PWM_bytes[1]
        start_motor_command = MOTOR_START_BYTE + PWM_payload_size + PWM_payload_data + PWM_checksum.to_bytes(1,'big')
        self._send_cmd(start_motor_command)

    def start_motor(self):
        '''Starts sensor motor'''
        self.logger.info('Starting motor')
        #for A1 only
        self._serial_port.setDTR(False)     #idk why but this have to be False to make A2 rotates
        
        #for A2, send command to start motor. MOTOR_START_BYTE = 0xF0
        self.motor_setPWM(DEFAULT_MOTOR_PWM)
        #time.sleep(1)   #delay for 1 second for motor to start running
    ...
    def stop_motor(self):
        ...
        #self._serial_port.setDTR(True)     #to stop A1 motor
        #to stop A2 motor, PWM set to 0
        self.motor_setPWM(0)
